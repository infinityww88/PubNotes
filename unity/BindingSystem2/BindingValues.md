Binding System 2 的设计初衷就是让你能够尽可能快速、简单地建立对象间的关联。在大多数脚本、对象、材质和组件中，大部分字段都开箱即用地支持绑定。只要某个字段是可序列化的，并且其绘制器包含 BeginProperty 调用，那么它很可能就可以被绑定。默认情况下，所有使用标准绘制器的脚本字段都是可绑定的，这意味着你使用 Unity 标准绘制器编写的脚本以及第三方脚本都能完全兼容。
