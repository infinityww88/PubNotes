...
在将 Google 移动广告 SDK 部署到 Android 或 iOS 平台时，你的 Unity 应用中现已包含该 SDK。现在，你可以植入广告了。AdMob 提供了许多不同的广告格式，你可以从中选择一种能够提供最佳用户体验的广告格式。
...
- 插页式广告
  ...
  插页式广告单元会在你的应用中展示全屏广告。请将其放置在应用界面中的自然停顿点和过渡点，例如在游戏应用中通过关卡后。

- 原生

  借助原生广告，你可以自定义素材资源（如标题和号召性用语）在应用中的呈现方式。通过自行设置广告样式，你可以呈现出自然、不突兀的广告展示效果，从而使用户体验更加丰富。

- 激励

  激励广告单元可让用户玩游戏、填写调查问卷或观看视频，以赢取应用内奖励，例如金币、额外的生命或积分。你可以为不同的广告单元设置不同的奖励，并指定用户获得的奖励价值和奖品。

- 插页式激励广告

  插页式激励广告是一种激励用户的新型广告格式，可让你通过在应用中的自然过渡点自动展示的广告向用户提供奖励，例如金币或额外生命值。
...
- 打开应用
  ...
  开屏广告是一种在用户打开或切换回你的应用时显示的广告格式。这种广告会叠加在加载屏幕上。
...
应根据平台选择使用 Google 提供的不同测试广告单元。在 iOS 设备上发出测试广告请求时你需要使用 iOS 测试广告单元，而在 Android 设备上发出请求时，需要使用 Android 测试广告单元。
...
- 在 AdMob 界面中添加测试设备
  ...
  新的测试设备通常会在 15 分钟内开始在你的应用中投放测试广告，但最多可能需要 24 小时。
...
- 以编程方式添加测试设备
  ...
  - 重新运行应用。如果你将设备正确添加为测试设备，则会在横幅广告、插页式广告和激励广告顶部的中间部分看到一个 Test Ad 标签。

    凡是带有 Test Ad 标签的广告，均可以放心点击。对测试广告的任何请求、展示和点击都不会显示在你帐号的报告中。
...
注意 ：由于模拟广告是 Prefab 对象，因此它们会显示在 Unity 编辑器中 GUI 的“下方”。但是，当你导出到移动平台时，广告会呈现在任何游戏视图之上。
...
- 加载激励广告
  ...
  提示：你可以使用广告加载调用在打算展示广告之前构建预加载广告的缓存，以便在需要时以零延迟展示广告。由于广告会在一小时后过期，因此你应该清除此缓存，并且每小时重新加载一次新广告。
...
- 监听激励广告事件
  ...
  若要进一步自定义你广告的行为，你可以在广告生命周期内加入许多事件，如打开、关闭等等。你可以通过注册代理来监听这些事件，如下所示。
